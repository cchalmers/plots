...
barPlotNormal :: (Typeable b, Renderable (Path V2 Double) b,
                  Applicative m, MonadState (Axis b c Double) m,
                  BaseSpace c ~ V2) =>
                 Int -> [Double] -> Double -> m ()
...
barPlotNormalC y xs cs w =
  F.for_ (sortBy (compare `on` fst)(zip xs cs)) $
...
barPlotNormalCL y xs cs ls w =
  F.for_ (sortBy (compare `on` fstof3)(zip3 xs cs ls)) $
...
barPlotStackedC y xs cs w =
  F.for_ (sortBy (compare `on` fst) (zip (additive xs) cs)) $
    \x -> barPlotC ((fromIntegral y), fst x) w (snd x)
...
barPlotStackedCL y xs cs ls w =
  F.for_ (sortBy (compare `on` fstof3) (zip3 (additive xs) cs ls)) $
    \x -> barPlotL ((fromIntegral y), fstof3 x) w (trdof3 x) (sndof3 x)
...
barPlotSplitC y xs cs w =
  F.for_ (zip (zip (createsplitdata y len w) xs) cs) $ \x -> barPlotC (fst x) w1 (snd x)
  where
    w1  = w/fromIntegral len
    len = length xs
...
barPlotSplitCL y xs cs ls w =
  F.for_ (zip3 (zip (createsplitdata y len w) xs) cs ls) $ \x -> barPlotL (fstof3 x) w1 (trdof3 x) (sndof3 x)
  where
    w1  = w / fromIntegral len
    len = length xs
