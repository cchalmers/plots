...
instance (Renderable (Path V2 n) b, HasLinearMap v, TypeableFloat n)
...
mkMark :: (HasThemeEntry a b n, TypeableFloat n, HasGenericPlot a b) => a -> Diagram b V2 n
mkMark a = (a ^. themeMarker) # applyStyle (a ^. themeMarkerStyle)

-- | Prism specific to a 'ScatterPlot'.
_ScatterPlot :: Plotable (ScatterPlot b v n) b => Prism' (Plot b v n) (ScatterPlot b v n)
_ScatterPlot = _Plot
